...
void World::LoadConfigSettings(bool reload)
{
    ...
    //Resetear CD y vida en duelos
    _bool_configs[CONFIG_DUEL_RESET_COOLDOWN_ON_START] = sConfigMgr->GetOption<bool>("DuelReset.Cooldown.OnStart", false);
    _bool_configs[CONFIG_DUEL_RESET_COOLDOWN_ON_FINISH] = sConfigMgr->GetOption<bool>("DuelReset.Cooldown.OnFinish", false);
    _bool_configs[CONFIG_DUEL_RESET_COOLDOWN_ONLY_IN_ZONA_DE_DUELOS] = sConfigMgr->GetOption<bool>("DuelReset.Cooldown.Only.in.zona.de.duelos", false);
    _bool_configs[CONFIG_DUEL_RESET_COOLDOWN_RESET_ENERGY_ON_START] = sConfigMgr->GetOption<bool>("DuelReset.Cooldown.Reset.Energy.OnStart ", false);
    _bool_configs[CONFIG_DUEL_RESET_COOLDOWN_MAX_ENERGY_ON_START] = sConfigMgr->GetOption<bool>("DuelReset.Cooldown.Max.Energy.OnStart", false);
    // Arenas 1v1
    _bool_configs[CONFIG_ARENA_1V1_ENABLE] = sConfigMgr->GetOption<bool>("Arena.1v1.Enable", true);
    _bool_configs[CONFIG_ARENA_1V1_ANNOUNCER] = sConfigMgr->GetOption<bool>("Arena.1v1.Announcer", false);
    _int_configs[CONFIG_ARENA_1V1_MIN_LEVEL] = sConfigMgr->GetOption<int32>("Arena.1v1.MinLevel", 80);
    _int_configs[CONFIG_ARENA_1V1_COSTS] = sConfigMgr->GetOption<int32>("Arena.1v1.Costs", 400000);
...
/// Send a packet to all players (or players selected team) in the area (except self if mentioned)
void World::SendAreaMessage(uint32 area, WorldPacket* packet, ...)
{
    SessionMap::const_iterator itr;
    for (itr = _sessions.begin(); itr != _sessions.end(); ++itr)
    {
...
/// Send a System Message to all players in the area (except self if mentioned)
void World::SendAreaText(uint32 area, const char* text, .../*WorldSession* self, uint32 team*/)
{
    char str[1024];
    va_list ap;
    va_start(ap, text);
    ...
    SendAreaMessage(area, &data/*, self, team*/);
}

/// Send a packet to all players
void World::SendAllWorldMessage(WorldPacket* packet, ...)
{
    SessionMap::const_iterator itr;
    for(itr = _sessions.begin(); itr != _sessions.end(); ++itr)
    {
...
void World::SendAllWorldText(const char* text, .../*WorldSession* self, uint32 team*/)
{
    char str[1024];
    va_list ap;
    va_start(ap, text);
    ...
    data << msg.c_str();
    data << (uint8)0;
    SendAllWorldMessage(&data);
}

/// Send a packet to all players
void World::SendPlayerMessage(WorldPacket* packet, ...)
{
	/*Player* player;
...
/// Send a System Message to all players
void World::SendPlayerText(Player* player, const char* text, ...)
{
    char str[1024];
    va_list ap;
    va_start(ap, text);
