...
uint8 Battleground::ClickFastStart(Player* player, GameObject* go)
{
    ...
    std::set<uint64>::iterator pIt = m_playersWantsFastStart.find(player->GetGUID().GetRawValue());
    ...
    m_playersWantsFastStart.insert(player->GetGUID().GetRawValue());
    ...
    if (sBattlegroundMgr->isArenaTesting() && isArena())
        playersNeeded = 2;
